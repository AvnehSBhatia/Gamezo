...
import { PromptSuggestions } from "@/components/gamezo/game/prompt-suggestions";
    ...
    <main className="relative min-h-screen overflow-hidden bg-[#fffdf8] px-4 pb-8 text-neutral-950">
      ...
      <header className="relative z-10 mx-auto flex max-w-7xl items-center justify-between py-5">
        ...
        <div className="hidden rounded-3xl border border-neutral-200 bg-white px-6 py-3 text-xl font-black shadow-md sm:flex sm:items-center sm:gap-3">
          <span className={`h-5 w-5 rounded-full ${opponentPromptLocked ? "bg-green-500" : "bg-yellow-400"}`} />
          {opponentPromptLocked ? "Opponent locked in" : "Opponent writing…"}
        </div>
      ...
      <section className="relative z-10 mx-0 grid w-full max-w-none items-start gap-5 lg:mx-auto lg:max-w-7xl lg:grid-cols-[22rem_1fr_22rem]">
        ...
        <article className="min-w-0 rounded-[2rem] border-2 border-neutral-950 bg-white p-5 shadow-[0_18px_42px_rgba(15,23,42,0.08)] sm:p-7">
          <div className="mb-3 rounded-2xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm font-bold text-blue-700">
          ...
          <div className="relative">
            <textarea
              ...
              disabled={selfPromptLocked}
              className="min-h-44 w-full resize-none rounded-3xl border-2 border-blue-500 bg-white px-5 py-5 font-mono text-xl font-bold leading-relaxed outline-none focus:ring-4 focus:ring-blue-100 disabled:opacity-60 sm:text-2xl"
              maxLength={200}
          ...
          <PromptSuggestions activePrompt={prompt} disabled={selfPromptLocked} onSelect={onPromptChange} />
        ...
        <PlayerCameraCard
          ...
          note={opponentPromptLocked ? "Opponent: locked" : "Opponent: writing…"}
        />
      </section>
